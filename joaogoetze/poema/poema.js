...
function mostrarEEsconderPoemado() {
  poemaElement.textContent = poema;

  // Divide o poema em palavras para um efeito mais gradual
  const palavras = poema.split(' ');

  // Função recursiva para remover uma palavra por vez
  function removerPalavras() {
    if (palavras.length > 0) {
      palavras.pop(); // Remove a última palavra
      poemaElement.textContent = palavras.join(' ');
      setTimeout(removerPalavras, 100); // Chama a si mesma a cada 100ms
    }
  }

  setTimeout(removerPalavras, 5000); // Inicia o processo de remoção após 5 segundos
}

mostrarEEsconderPoemado();